#Programa FDTD_2d PEC  mejorado representando en 2D el plano XZ
import numpy as np
import math
...
for n in range(1,lim_step+1):
    ...
    H_x[0:Ni,Nj-1] = C_Hx * H_x[0:Ni,Nj-1] - D_Hx * (- E_z[0:Ni,Nj-1]) / As
    H_y[0:Ni-1,0:Nj] = C_Hy * H_y[0:Ni-1,0:Nj] - D_Hy * (E_z[0:Ni-1,0:Nj] - E_z[1:Ni,0:Nj]) / As
    H_y[Ni-1,0:Nj] = C_Hy * H_y[Ni-1,0:Nj] - D_Hy * ( E_z[Ni-1,0:Nj]) / As
    #Actualización del campo E con condiciones de contorno PEC
    E_z[0,0] = C_Ez * E_z[0,0] + D_Ez * (H_y[0,0] - H_x[0,0]) / As
    E_z[1:Ni,0] = C_Ez * E_z[1:Ni,0] + D_Ez * (H_y[1:Ni,0] - H_y[0:Ni-1,0] - H_x[1:Ni,0]) / As
    E_z[0,1:Nj] = C_Ez * E_z[0,1:Nj] + D_Ez * (H_y[0,1:Nj] + H_x[0,0:Nj-1] - H_x[0,1:Nj]) / As
    E_z[1:Ni,1:Nj] = C_Ez * E_z[1:Ni,1:Nj] + D_Ez * (H_y[1:Ni,1:Nj] - H_y[0:Ni-1,1:Nj] + H_x[1:Ni,0:Nj-1] - H_x[1:Ni,1:Nj]) / As
    E_z[15,15] = E_z[15,15] + gauss
    
    # Almacena los valores de E_z[:,15]
    ez_values_at_15.append(E_z[:, 15].copy())
    plt.clf()
    # Gráfico de E_z[:,15] en cada iteración
    plt.plot(x_positions, ez_values_at_15[-1], label=f'E_z[:,15] - Tiempo: {n}')
    plt.xlabel('Posición en el eje x')
    plt.ylabel('E_z[:,15]')
    plt.title(f'Evolución de E_z[:,15]')
    plt.grid(True)

    # Establece límites del eje y
    plt.ylim(-0.2, 0.25)
    # Actualiza y muestra el gráfico en cada iteración
    display(plt.gcf())
    clear_output(wait=True)
    time.sleep(0.05)  # Pausa entre iteración
