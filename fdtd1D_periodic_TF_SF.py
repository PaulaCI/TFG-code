#Programa FDTD_1d Condiciones de contorno periódicas (fuente suave TF/SF)
import numpy as np
import math
...
#Almacena el campo elé©ctrico en cada iteración
...
for n in range(1,lim_step+1):
    ...
    #ActualizaciÃ³n del campo H con condiciones de contorno periÃ³dicas
    H_x[:Nk-1] = C_Hx * H_x[:Nk-1] - D_Hx * (E_y[:Nk-1] - E_y[1:Nk]) / As
    H_x[15] = H_x[15] - D_Hx * (gauss) / As
    H_x[Nk-1] = C_Hx * H_x[Nk-1] - D_Hx * (E_y[Nk-1] - E_y[0]) / As
    #ActualizaciÃ³n del campo E con condiciones de contorno periÃ³dicas
    ...
    if n % 2 == 0:
        ...
        # Actualiza y muestra el grÃ¡fico en cada iteración
        display(plt.gcf())
        if n == 184:
            break
